...
import { blockExternalAuthRequests } from "./memory-test-helpers";
...
test.describe("memory - node settings @settings", () => {
  test.beforeEach(async ({ page }) => {
    await blockExternalAuthRequests(page);
  });

  test("Node settings panel is visible with declared controls", async ({
    page
  }, testInfo) => {
