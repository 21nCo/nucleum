<script lang="ts">
    ...
    reader.onload = (event) => {
      try {
        ...
        if (isValidUserPreferences(jsonData)) {
          ...
          let sessionStorage = window?.localStorage.getItem("Sessions");
          if (sessionStorage) {
  ...
  function isValidUserPreferences(item: any): item is UserGlobalPreferences {
    return (
      item &&
...
<div class="flex flex-col gap-8 w-full">
  <div class="flex flex-col gap-1">
    ...
    <div class="flex gap-2">
      ...
      />
      <Button label={"Export preferences (JSON)"} on:click={exportPreferencs} />
    </div>
  </div>
  <div class="flex flex-col gap-1">
    <div>Import</div>
    <div class="flex gap-2">
      ...
      />
      <Button on:click={() => fileInput.click()} label={"Import preferences"} />
    </div>
  </div>
  <div>
