...
def performRootQuery(params):
    print("performing root query:", os.getenv(
        'DOMAIN'), params['region'])
    headers = {
        "Content-Type": "text/plain",
        ...
        "DB": os.getenv('ADMIN_DB_NAME', 'ADMIN') if params['dbType'] == CONTEXT['ADMIN'] else params['db'],
    }
    body = params['query']
    ...
    #     "headers": headers,
    # })
    end_point = "https://" + params["region"] + ".db." + os.getenv('DOMAIN') + "/sql"
    response = requests.post(end_point, headers=headers, data=encoded_body)
