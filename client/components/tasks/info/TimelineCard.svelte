<script lang="ts">
  ...
  $: spanScale = $task.spanScale || TimeScale.DAYS;
  ...
  function handleDateChange(e: CustomEvent<{ start: Date; end: Date }>) {
    $task.startDate = e.detail.start;
    $task.endDate = e.detail.end;
    task.modify({
...
<div class="flex flex-col gap-4 rounded-md border border-brs3 px-3 py-4">
  ...
  <div class="flex flex-col gap-2">
    <div class="relative px-6">
      <TimelineCardAxis {startDate} {endDate} {spanScale} />
    </div>
  </div>
