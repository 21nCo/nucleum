...
import { RemotePersistenceProvider, type PersistenceProvider } from "$lib/client/persistence/persistence.type";
...
export async function delegateToFlux(method: IFluxMethod) { 
    try {
        logger.debug({at: "delegateToFlux", method});
        ...
        switch(method.method) {
            ...
            case FluxMethod.INIT_FLUX:
                return initFlux(method.args.stores, method.args.provider, new DexiePersistence(RemotePersistenceProvider.SURREAL), method.args.userId, method.args.params);
