<script lang="ts">
  ...
  export let value: IRecordId;

  let isUploadInProgress = false;

  let selectedMethod: string = "gradient";
  let _value = transformValue(value);
  let imageFilesFromLibrary: IFile[] = [];

  onMount(async () => {
    imageFilesFromLibrary = await fetchImageFilesFromLibrary();
    console.log(imageFilesFromLibrary);
  });

  function transformValue(value: IRecordId) {
...
<div class="flex flex-col gap-6 w-full h-full p-6">
  <div class="flex flex-row items-center justify-between">
    <Text content="Choose image" style={TextStyle.PANEL_HEADING} />
    <Button icon="cross" on:click={onClose} />
  </div>
  <div class="h-40 w-full py-8">
    <div
  ...
  <div class="flex flex-col gap-4 w-full flex-1 overflow-auto">
    <PanelSwitcher
    ...
    {:else if selectedMethod === "gradient"}
      <GradientsSelector on:change={handleGradientChange} bind:value={_value} />
